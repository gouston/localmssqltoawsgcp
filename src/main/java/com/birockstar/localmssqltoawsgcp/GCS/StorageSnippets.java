/*
 * 2019 Mike Houston birockstar.com
 *
...
* Storage's javadoc.
*/
package com.birockstar.localmssqltoawsgcp.GCS;
...
public class StorageSnippets {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        /*
         * Storage storage1; storage1 =
         * StorageOptions.getDefaultInstance().getService();
         * 
         * String bucketName = "some_bucket"; String blobName =
         * "sample_load_jsonfile.json";
         */

        String PROJECT_ID = "some-project-1234";
        String PATH_TO_JSON_KEY = "";
        String BUCKET_NAME = "some_bucket";
        String OBJECT_NAME = "subfolder/sample_load_jsonfile.json";
        ...
        /*
         * try { new StorageSnippets(storage).readerFromIdDoSomething(BUCKET_NAME,
         * OBJECT_NAME); } catch (IOException e) { // TODO Auto-generated catch block
         * e.printStackTrace(); }
         */

        System.out.println(new StorageSnippets(storage).readFileContents(BUCKET_NAME, OBJECT_NAME));

    }

    public String readFileContents(String bucketName, String blobName) throws IOException {
        // [START readerFromId]
        BlobId blobId = BlobId.of(bucketName, blobName);

        String contents = null;
        // System.out.println("in sub routine, bucket is: " + bucketName + " and object
        // name is: " + blobName);
        ...
        try (ReadChannel reader = storage.reader(blobId)) {
            ...
            // System.out.println("total bytes to read are: " + reader.read(bytes));
            while (reader.read(bytes) > 0) {
                // System.out.println("iterating bytes");

                bytes.flip();

                /*
                 * BufferedReader in = new BufferedReader(new
                 * InputStreamReader(Channels.newInputStream(reader))); String inputLine; //
                 * StringBuilder response = new StringBuilder(); while ((inputLine =
                 * in.readLine()) != null) { System.out.println(inputLine); } in.close();
                 */

                contents = StandardCharsets.UTF_8.decode(bytes).toString();
    ...
    // [VARIABLE "my_unique_bucket"]
    // [VARIABLE "my_blob_name"]
    public Blob createBlobFromByteArray(String bucketName, String blobName, String objectPath) {
        // [START createBlobFromByteArray]
        BlobId blobId = BlobId.of(bucketName, blobName);
        BlobInfo blobInfo = BlobInfo.newBuilder(blobId).setContentType("text/plain").build();
        Blob blob = storage.create(blobInfo, PutObject.getObjectFile(objectPath));
...
/*    public Blob createBlobFromInputStream(String bucketName, String blobName) {
    ...
    public List<String> listOfBlobsWithDirectoryAndPrefix(String bucketName, String directory) {
        // [START listBlobsWithDirectoryAndPrefix]
        ...
        for (Blob blob : blobs.iterateAll()) {
            // do something with the
            sourceUris.add(bucketName + directory + blob.getName());
        }
        // [END listBlobsWithDirectoryAndPrefix]
        return sourceUris;
    }
    ...
    public Blob copyBlob(String srcBucket, String destBucket, String blobName, String copyBlobName) {
        // [START copyBlob]
        CopyRequest request
                = CopyRequest.newBuilder()
                        .setSource(BlobId.of(srcBucket, blobName))
                        .setTarget(BlobId.of(destBucket, copyBlobName))
                        .build();
        Blob blob = storage.copy(request).getResult();
        // [END copyBlob]
        return blob;
    ...
    public List<Acl> listBucketAcls(String bucketName) {
        ...
        for (Acl acl : acls) {
            System.out.println(acl.getEtag());
